...
import { getUserTasks, getMemberTaskDetails } from "../../../services";
import { PopUp } from "./components";

export function AllTask(props) {
  ...
  useEffect(() => {
    getUserTasks(page, count, Token)
      .then((data) => {
        setTasks(data.task);
        setCountOfTasks(data.countOfTasks);
      })
      .catch(() => console.log("Invalid fetch"));
            ...
            <tr
              ...
              onClick={() => {
                getMemberTaskDetails(el.id)
                  ...
                  .catch((e) => console.log(e, "Error in progress FETCH"));
                setOpenPopUp(el.id);
              }}
            >
              <td>{el.id}</td>
